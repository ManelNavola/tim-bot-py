...
#
#     current_enemy_rows = [x for x in enemy_rows]
#
...
#
#     def update_table():
#         nonlocal current_enemy_rows, table
#         current_enemy_rows = [x for x in enemy_rows]
#
#         def sort_stats(x):
#             stats = x[2:2 + len(Stats.get_all())]
#             trs = 0
#             for stat in stats:
#                 if stat:
#                     trs += int(stat)
...
#
#         if sort_by == 'Stats':
#             current_enemy_rows.sort(key=sort_stats)
#         table.update([x for x in current_enemy_rows if contains.lower() in x[1].lower()])
#
#     gui = [
#         [Sg.Text("Sort by:"),
#          Sg.Combo(['None', 'Stats'], 'None',
#                   size=(15, 1), enable_events=True, key='Sort', readonly=True),
#          Sg.Text("Contains:"),
#          Sg.InputText('', size=(20, 1), enable_events=True, key='Contains'),
#          Sg.HSeparator(),
#          Sg.Button("Consolidate", key='Cons'),
#          Sg.Button("(-) Remove Enemy", key='Remove'),
...
#                 new_row = modify_enemy_row(current_enemy_rows[index])
#                 if new_row is not None:
#                     for i in range(len(enemy_rows)):
#                         if enemy_rows[i] == new_row[0]:
#                             enemy_rows[i] = new_row
...
#                 index = values['Table'][0]
#                 if index:
#                     row: list[str] = current_enemy_rows[index]
#                     if row is not None:
#                         if row[0].startswith('X'):
#                             if Sg.popup_yes_no(f"Are you sure you want to delete {row[1]}?") == 'Yes':
#                                 for i in range(len(enemy_rows)):
#                                     if enemy_rows[i][0] == row[0]:
#                                         del enemy_rows[index]
#                                         changes = True
#                                         update_table()
#                         else:
#                             Sg.popup_ok("You cannot delete a consolidated enemy")
#         elif event == 'Save':
...
#                 if event2 == 'OK' or event2 == Sg.WINDOW_CLOSE_ATTEMPTED_EVENT:
#                     if changes:
#                         is_yes = Sg.popup_yes_no("Are you sure you want to exit without saving?")
#                         if is_yes == 'Yes':
#                             break
#                     else:
#                         break
