...
#          Sg.Button("(-) Remove Item", key='Remove'),
...
#                 new_row = modify_item_row(current_item_rows[index])
#                 if new_row is not None:
#                     for i in range(len(item_rows)):
#                         if item_rows[i][0] == new_row[0]:
#                             item_rows[i] = new_row
...
#                 index = values['Table'][0]
#                 if index:
#                     row: list[str] = current_item_rows[index]
#                     if row is not None:
#                         if row[0].startswith('X'):
#                             if Sg.popup_yes_no(f"Are you sure you want to delete {row[1]}?") == 'Yes':
#                                 for i in range(len(item_rows)):
#                                     if item_rows[i][0] == row[0]:
#                                         del item_rows[i]
#                                         modified.add(i)
#                                         update_table()
#                         else:
#                             Sg.popup_ok("You cannot delete a consolidated item")
#         elif event == 'Save':
