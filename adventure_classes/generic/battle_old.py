...
class BattleTeamCache:
    def __init__(self, lang: str, name: str, entities: list[Entity], affiliation: Optional[int] = None):
        self.saved_lang: str = lang
        self.saved_name: str = name
        self.saved_entities: list[Entity] = entities
        self.saved_affiliation: Optional[int] = affiliation


class BattleTeam:
    def __init__(self, lang: str, name: str, entities: list[Entity], affiliation: Optional[int] = None):
        self.cache = BattleTeamCache(lang, name, entities, affiliation)

        self.entities: list[BattleEntity] = []
        self.users: list[BattleEntity] = []
        self.entity_dict: dict[Entity, BattleEntity] = {}
        self.user_acted: set[BattleEntity] = set()
        self.bots: list[BattleEntity] = []
        self.name: str = ''
        self.affiliation: Optional[int] = None
        self.joint_speed: float = 0
        self.speed_advantage: float = 1.0
        self.last_target: dict[BattleEntity, BattleEntity] = {}
        self.active_abilities: list[AbilityInstance] = []

    def init(self, adventure: Adventure):
        self.entities = [BattleEntity(entity, self.cache.saved_lang) for entity in self.cache.saved_entities]
        self.users = [be for be in self.entities if be.is_user()]
        self.entity_dict = {be.entity: be for be in self.entities}
        self.bots = [be for be in self.entities if not be.is_user()]
        self.name = self.cache.saved_name
        self.affiliation: int = self.cache.saved_affiliation
        self.joint_speed = sum([be.entity.get_stat_value(Stat.SPD) for be in self.entities]) / len(self.entities)
...
class BattleTeamUsers(BattleTeam):
    def __init__(self, lang: str, affiliation: Optional[int] = None):
        super().__init__(lang, '', [], affiliation)

    def init(self, adventure: Adventure):
        self.cache.saved_entities = [user.user_entity for user in adventure.get_users()]
        self.cache.saved_name = ', '.join([user.get_name() for user in adventure.get_users()])
        super().init(adventure)
...
class BattleChapter(Chapter):
    ...
    def __init__(self):
        ...
        self._active_abilities: list[AbilityInstance] = []
    ...
    async def init(self) -> None:
        ...
        actions: set[BattleEmoji] = {BattleEmoji.ATTACK}
        for team in self._teams:
        ...
        for action in actions:
            await self.get_adventure().add_reaction(action.value, self.execute_action)
    ...
    async def _perform_action(self, be: BattleEntity, action: BattleEmoji, targets: list[BattleEntity])\
            -> str:
        msg: str = be.try_action(action, targets)
        ...
        if user_be:
            if user_be not in self._current_team.user_acted:
                ...
                if target_entity is not None and target_entity.is_dead():
                    return
                msg = await self._perform_action(user_be, BattleEmoji(emoji), [target_entity])
                if self._finished:
                    return
                if msg:
    ...
    async def next_turn(self):
        ...
        # Turn FX
        lc = self._current_team.active_abilities
        self._current_team.active_abilities = []
        for instance in lc:
            instance.duration_remaining -= 1
            if instance.duration_remaining > 0:
                self._current_team.active_abilities.append(instance)
                instance.ability.turn(instance.targets)

        if (not self._current_team.users) or (not self._current_team.any_user_alive()):
            # No users: skip to bots
        ...
        if self._current_team.any_bot_alive():
            ...
            for bot in self._current_team.bots:
                if not bot.is_dead():
                    ...
                    msg: str = await self._perform_action(bot, BattleEmoji.ATTACK, target)
                    if msg:
                        self._last_messages.append(msg)
...
def q1vp(lang: str, a: Entity, battle: Optional[BattleChapter] = None) -> BattleChapter:
    if not battle:
        battle = BattleChapter()
    battle.add_team(BattleTeam(lang, a.get_name(), [a]))
    battle.add_team(BattleTeamUsers(lang))
    return battle
